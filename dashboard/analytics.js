/** Server-side mock GA4 + GSC analytics (shared logic with public/mock-analytics.js). */
...
function buildGsc(seed, days) {
  ...
  const pages = ['/', '/platform/meta', '/platform/google', '/platform/tiktok', '/utm-generator', '/blog', '/platform/shopee'].map(
    (page, i) => {
      ...
      return {
        ...
        clicks,
        ctr: +(clicks / impressions).toFixed(4),
        position: +(12 + i * 2.1 - r * 3).toFixed(1),
      };
    }
  );
...
function buildCombined(ga4, gsc) {
  ...
  const landingPerformance = [...pageMap.values()]
    .map((row) => {
      ...
      return { ...row, efficiency: sessions > 0 ? +(clicks / sessions).toFixed(3) : 0, signal };
    })
...
export function getAnalytics({ from, to, preset } = {}) {
  let range;
  if (from && to) {
  ...
  return {
    ...
    deltas: {
      ...
      avgPosition: delta({ position: gsc.totals.position }, { position: prevGsc.totals.position }, 'position'),
    },
    disclosure:
